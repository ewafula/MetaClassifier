...
* `<FASTA_DIR>` is the directory containing samples read data fasta files produced by the MetaClassifier's `process_reads.py` script or from external sources for classification by the MetaClassifier's `classiify_reads.py` script. 
